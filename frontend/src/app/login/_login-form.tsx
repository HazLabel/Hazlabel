    ...
    const handleAuth = async (e: React.FormEvent) => {
        ...
        try {
            if (isSignUp) {
                ...
                toast.success("Welcome back!", { description: "Redirecting to your vault..." })
                setTimeout(() => {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-slate-50 relative px-6">
            ...
            <div className="relative w-full max-w-md">
                    ...
                    <form onSubmit={handleAuth} className="space-y-4">
                        ...
                        <div className="space-y-2">
                            <div className="flex justify-between">
                                ...
                                {!isSignUp && (
                                    <Link href="#" className="text-sm text-sky-600 hover:text-sky-700 transition-colors">
                                        Forgot password?
                                    </Link>
                                )}
                            </div>
                            ...
                            {isSignUp && passwordStrength && (
                                <div className="flex items-center gap-2 text-xs">
                                    ...
                                    <span className={`font-medium ${passwordStrength === "weak" ? "text-red-600" : passwordStrength === "medium" ? "text-amber-600" : "text-emerald-600"}`}>
                        ...
                        {isSignUp && (
                            <div className="flex items-center space-x-3 py-3 animate-reveal">
                                ...
                                />
                                <label htmlFor="terms" className="text-sm text-slate-500 font-normal cursor-pointer select-none leading-relaxed">
                                    ...
                                    <Link href="/disclaimer" className="text-sky-600 hover:underline">Safety Disclaimer</Link>.*
                                </label>
                            </div>
                        )}
                ...
                {/* Trust signals */}
                <div className="mt-6 grid grid-cols-3 gap-3 animate-reveal delay-200">
                    <div className="bg-white/80 backdrop-blur rounded-xl border border-slate-200 p-3 text-center">
                        <p className="text-xs font-semibold text-slate-700">GHS Rev. 11</p>
                        <p className="text-xs text-slate-500 mt-0.5">Always current</p>
                    </div>
                    <div className="bg-white/80 backdrop-blur rounded-xl border border-slate-200 p-3 text-center">
                        <p className="text-xs font-semibold text-slate-700">OSHA HCS</p>
                        <p className="text-xs text-slate-500 mt-0.5">Compliant</p>
                    </div>
                    <div className="bg-white/80 backdrop-blur rounded-xl border border-slate-200 p-3 text-center">
                        <p className="text-xs font-semibold text-slate-700">SOC 2</p>
                        <p className="text-xs text-slate-500 mt-0.5">Encrypted data</p>
                    </div>
                </div>

                {/* Footer Credits */}
                <p className="text-center text-xs text-slate-400 mt-6 animate-reveal delay-300">
