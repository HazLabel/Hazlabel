...
Pictograms are critical to GHS hazard classification on chemical labels. Each [pictogram](/blog/ghs-classification-of-chemicals) consists of a symbol on a white background framed within a red border and represents distinct hazards. The chemical hazard classification determines the pictogram on the label. Each pictogram may only appear once on a label, even if multiple hazards require the use of the same pictogram.
